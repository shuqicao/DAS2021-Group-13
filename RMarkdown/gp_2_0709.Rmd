...
#View the outlier in aroma, flavor, acidity = 0 is Honduras
...
[1] "aroma"                "flavor"               "acidity"             
[4] "category_two_defects" "altitude_mean_meters" "Qualityclass"
...
qchisq(df = 10, p = 0.95)
```
It can be perceived that the largest reduction in residual deviance comes from aroma(472.06) and the smallest reduction in residual deviance is altitude_mean_meters(6.35). We could try a model without altitude_mean_meters as the resulting reduction in deviance(6.35) is smaller than the 95th percentile of $X^2(10)$ distribution 18.30704. This will be m4 model which includes aroma, flavor, acidity.
...
```{r test}
plot_model(m3, type = 'pred', terms = c("flavor", "aroma", "acidity", "altitude_mean_meters"))
```
